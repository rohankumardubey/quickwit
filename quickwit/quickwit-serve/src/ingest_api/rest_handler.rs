...
use quickwit_config::NATIVE_INGEST_SOURCE_ID;
use quickwit_ingest::{
    CommitType, DocBatchBuilder, DocBatchV2, FetchResponse, IngestErrorV2, IngestRequest,
    IngestRequestV2, IngestResponse, IngestResponseV2, IngestRouterService,
    IngestRouterServiceClient, IngestService, IngestServiceClient, IngestServiceError,
    IngestSubrequest, TailRequest,
};
...
pub(crate) fn ingest_api_handlers(
    ...
    ingest_handler(ingest_service.clone())
        ...
        .or(ingest_v2_handler(metastore.clone(), ingest_router.clone()))
}
...
async fn ingest_v2(
...
) -> Result<IngestResponseV2, IngestErrorV2> {
    ...
    // FIXME: that's no bueno for a high QPS service.
    let index_uid = metastore
        .index_uid(&index_id)
        .await
        .map_err(<quickwit_metastore::MetastoreError as Into<IngestErrorV2>>::into)? // FIXME
        .into();

    let subrequest = IngestSubrequest {
        index_uid,
        source_id: NATIVE_INGEST_SOURCE_ID.to_string(),
        doc_batch: Some(doc_batch),
    };
    let request = IngestRequestV2 {
        commit_type: CommitType::from(ingest_options.commit_type) as u32,
...
pub(crate) mod tests {
    ...
    use quickwit_ingest::{
        ...
        IngestApiService, IngestResponse, IngestRouterService, IngestRouterServiceClient,
        IngestServiceClient, MockIngestRouterService, SuggestTruncateRequest, QUEUES_DIR_NAME,
    };
    ...
    async fn test_ingest_api_returns_200_when_ingest_json_and_fetch() {
        ...
        let ingest_api_handlers =
            ingest_api_handlers(ingest_service, IngestRouterServiceClient::mock().into());
    ...
    async fn test_ingest_api_blocks_when_wait_is_specified() {
        ...
        let ingest_api_handlers = ingest_api_handlers(
            ...
            IngestRouterServiceClient::mock().into(),
        );
