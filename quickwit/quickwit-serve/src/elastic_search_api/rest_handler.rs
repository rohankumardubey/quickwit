...
pub fn es_compat_cluster_info_handler(
    ...
    let cluster_info_json = Arc::new(json!({
    ...
    elastic_cluster_info_filter().then(move || {
        let cluster_info_json_clone = cluster_info_json.clone();
        async move { Ok(warp::reply::json(&cluster_info_json_clone)) }
    })
}
