...
use crate::{
    AckReplicationMessage, CommitType, IngestErrorV2, ReplicateRequest, ReplicateResponse,
...
impl ReplicationClient {
    /// Sends a [`ReplicateRequest`] request to a follower and waits for a [`ReplicateResponse`].
    pub async fn replicate(
        &self,
        replicate_request: ReplicateRequest,
    ) -> super::Result<ReplicateResponse> {
        let (replicate_response_tx, replicate_response_rx) = oneshot::channel();
        self.oneshot_replicate_request_tx
            .clone()
            .send((replicate_request, replicate_response_tx))
            .expect("FIXME"); // TODO: wrap with timeout?
        let replicate_response = replicate_response_rx.await.expect("FIXME"); // TODO: wrap with timeout?
...
pub(super) struct ReplicationClientTask {
    ...
    ack_replication_stream: ServiceStream<super::Result<AckReplicationMessage>>,
...
impl ReplicationClientTask {
    ...
    async fn run(&mut self) {
        while let Some((replicate_request, replicate_response_tx)) =
            self.oneshot_replicate_request_rx.recv().await
        {
            ...
            let ack_replication_message = self
                ...
                .expect("FIXME")
                .expect("FIXME");
            let replicate_response = ack_replication_message
                .into_replicate_response()
                .expect("FIXME");
            replicate_response_tx
                .send(replicate_response)
                .expect("FIXME");
        }
    }
}
...
pub(super) struct ReplicationTaskHandle {
    _join_handle: JoinHandle<super::Result<()>>,
}
...
pub(super) struct ReplicationTask {
    ...
    ack_replication_stream_tx: mpsc::Sender<super::Result<AckReplicationMessage>>,
}
...
impl ReplicationTask {
    ...
    async fn run(&mut self) -> super::Result<()> {
        while let Some(syn_replication_message) = self.syn_replication_stream.next().await {
            let Some(replicate_request) = syn_replication_message.into_replicate_request() else {
                // The syn replication message is weirdly a `OpenReplicationStreamRequest` message.
                continue;
            };
            ...
            let commit_type = CommitType::from(replicate_request.commit_type);
            ...
            for subrequest in replicate_request.subrequests {
                ...
                let _replica_shard: &mut ReplicaShard =
                    if subrequest.from_position_exclusive.is_none() {
...
mod tests {
    ...
    use super::*;
    use crate::ingest_v2::fetch::FetchStreamManager;
    use crate::ingest_v2::test_utils::{MultiRecordLogTestExt, ReplicaShardTestExt};
    use crate::{queue_id, CommitType, DocBatchV2, ReplicateSubrequest, ReplicateSuccess};
    ...
    async fn test_replication_task_happy_path() {
        ...
        let state = Arc::new(RwLock::new(IngesterState {
            ...
            fetch_stream_manager: FetchStreamManager::new(leader_id.clone(), mrecordlog.clone()),
        }));
        ...
        syn_replication_stream_tx
            ...
            .await
            .unwrap();
        let replicate_response = ack_replication_stream
            .next()
            .await
            .unwrap()
            .unwrap()
