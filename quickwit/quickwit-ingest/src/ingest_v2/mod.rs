...
impl Shard {
    ...
    fn cmp_hash_key(&self, hash_key: u64) -> Ordering {
        if hash_key < self.start_hash_key_inclusive {
            return Ordering::Greater;
        }
        if hash_key < self.end_hash_key_exclusive || hash_key == u64::MAX {
...
impl AckReplicationMessage {
    ...
    fn into_replicate_response(self) -> Option<ReplicateResponse> {
        match self.message {
            Some(ack_replication_message::Message::ReplicateResponse(replicate_response)) => {
                Some(replicate_response)
            }
            _ => None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_shard_cmp_hash_key() {
        let mut shard = Shard {
            index_uid: "test-index".to_string(),
            source_id: "test-source".to_string(),
            leader_id: "test-leader-node".to_string(),
            end_hash_key_exclusive: 10,
            ..Default::default()
        };
        assert_eq!(shard.cmp_hash_key(0), Ordering::Equal);
        assert_eq!(shard.cmp_hash_key(9), Ordering::Equal);
        assert_eq!(shard.cmp_hash_key(10), Ordering::Less);

        shard.start_hash_key_inclusive = 1;
        assert_eq!(shard.cmp_hash_key(0), Ordering::Greater);
        assert_eq!(shard.cmp_hash_key(1), Ordering::Equal);

        shard.end_hash_key_exclusive = u64::MAX - 1;
        assert_eq!(shard.cmp_hash_key(u64::MAX - 2), Ordering::Equal);
        assert_eq!(shard.cmp_hash_key(u64::MAX - 1), Ordering::Less);

        shard.end_hash_key_exclusive = u64::MAX;
        assert_eq!(shard.cmp_hash_key(u64::MAX - 1), Ordering::Equal);
        assert_eq!(shard.cmp_hash_key(u64::MAX), Ordering::Equal);
    }
