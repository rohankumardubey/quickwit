...
use crate::{
    ...
    ReplicateRequest, ReplicateSubrequest, Shard, SynReplicationMessage, TruncateRequest,
    TruncateResponse, TruncateSubrequest, UpdateFetchStreamRequest, UpdateFetchStreamResponse,
};
...
impl IngesterState {
    pub(super) fn find_shard_status_rx(
        &self,
        queue_id: &QueueId,
    ) -> Option<watch::Receiver<ShardStatus>> {
        self.primary_shards
            .get(queue_id)
...
impl Ingester {
    pub async fn try_new(
        ...
        let mrecordlog = MultiRecordLog::open(wal_dir_path)
            .await
    ...
    async fn init_replication_client<'a>(
        ...
        let Entry::Vacant(entry) = state_guard.replication_clients.entry(follower_id.clone()) else {
            return Ok(());
        };
...
impl IngesterService for Ingester {
    ...
    async fn persist(&mut self, persist_request: PersistRequest) -> super::Result<PersistResponse> {
        ...
        let mut replicate_response_futures = FuturesUnordered::new();
        ...
        for (follower_id, subrequests) in replicate_subrequests {
            ...
            let replicate_handler = state_guard
                .replication_clients
                .get(&follower_id)
                .expect("The replicate handler should be initialized.")
                .clone();
            let replicate_response_future =
                async move { replicate_handler.replicate(replicate_request).await };
            replicate_response_futures.push(replicate_response_future);
        }
        ...
        while let Some(replicate_response_result) = replicate_response_futures.next().await {
            let replicate_response = replicate_response_result?;
    ...
    async fn open_replication_stream(
        ...
        let Entry::Vacant(entry) = state_guard.replication_tasks.entry(leader_id.clone()) else {
            return Err(IngestErrorV2::Internal(
                format!("A replication stream betwen {leader_id} and {follower_id} is already opened.")));
        };
    ...
    async fn truncate(
        ...
        for (follower_id, subrequests) in truncate_subrequests {
            ...
            let mut ingester = self.ingester_pool.get(&follower_id).await.expect("FIXME");
            let truncate_future = async move { ingester.truncate(truncate_request).await };
            truncate_futures.push(truncate_future);
        }
        while let Some(truncate_future_res) = truncate_futures.next().await {
            // TODO: Handle errors.
            truncate_future_res?;
        }
...
impl Default for ShardStatus {
    fn default() -> Self {
        ShardStatus::Open(Position::default())
    }
}
...
impl ShardStatus {
    pub fn replication_position_inclusive(&self) -> Position {
        match self {
            ShardStatus::Open(position) => *position,
            ShardStatus::Closed(position) => *position,
        }
    }
}
...
impl PrimaryShard {
    fn set_primary_position_inclusive(&mut self, primary_position_inclusive: impl Into<Position>) {
        ...
        if self.follower_id.is_none() {
            self.shard_status_tx
                .send(ShardStatus::Open(self.primary_position_inclusive))
                .expect("The channel should be open."); // because we always hold a sender.
        }
    }
    ...
    fn set_replica_position_inclusive(&mut self, replica_position_inclusive: impl Into<Position>) {
        ...
        self.shard_status_tx
            .send(ShardStatus::Open(replica_position_inclusive))
            .expect("The channel should be open."); // because we always hold a sender.
    }
}
...
impl ReplicaShard {
    pub fn set_replica_position_inclusive(
        ...
        self.shard_status_tx
            .send(ShardStatus::Open(self.replica_position_inclusive))
            .expect("The channel should be open."); // because we always hold a sender.
    }
}
