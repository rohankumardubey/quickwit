...
pub struct MultiFetchStream {
    ...
    fetch_streams: HashMap<QueueId, FaultTolerantFetchStream>,
...
impl MultiFetchStream {
    pub fn new(node_id: NodeId, client_id: String, ingester_pool: IngesterPool) -> Self {
    ...
    pub async fn subscribe(
        ...
        match entry {
            ...
            Entry::Vacant(entry) => {
                let mut fault_tolerant_fetch_stream = FaultTolerantFetchStream::new();
                fault_tolerant_fetch_stream
                    .subscribe(leader_id, follower_id, subscriptions)
                    .await?;
...
/// A fetch stream that that can sustain the failure of one ingester and fallback to another one.
struct FaultTolerantFetchStream {
    node_id: NodeId,
    client_id: String,
    ingester_pool: IngesterPool,
    fetch_payload_tx: mpsc::Sender<super::Result<FetchPayload>>,
...
//     ) -> super::Result<()> { let queue_id = queue_id(index_uid.as_str(), &source_id, shard_id);
//       if self.fetch_streams.contains_key(&queue_id) { unimplemented!("Update fetch stream is not
//       implemented.") } let node_id: NodeId = leader_id.into(); let mut ingester =
//       self.ingester_pool.get(&node_id).await.expect("FIXME"); let subscribe_request =
//       SubscribeToShard { index_uid: index_uid.into(), source_id, shard_id,
//       from_position_exclusive, to_position_inclusive: None, }; let open_fetch_stream_request =
//       OpenFetchStreamRequest { client_id: self.client_id.clone(), subscriptions:
//       vec![subscribe_request], }; let mut fetch_stream = ingester
//       .open_fetch_stream(open_fetch_stream_request) .await?;
...
mod tests {
    ...
    async fn test_fetch_stream() {
        ...
        shard_status_tx_00.send(shard_status).unwrap();
        ...
        shard_status_tx_01.send(shard_status).unwrap();
        let fetch_payload = timeout(Duration::from_millis(100), service_stream.next())
            .await
            .unwrap()
            .unwrap()
