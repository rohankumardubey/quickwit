...
mod tests {
    ...
    //         MultiRecordLog::open(tempdir.path()).await.unwrap(),
    //     ));
    //     let mut fetch_stream_manager = FetchStreamManager::new(node_id.clone(),
    // mrecordlog.clone());

    //     let client_id = "test-client".to_string();
    //     let (fetch_stream, mut service_stream) =
    // fetch_stream_manager.open_fetch_stream(&client_id);     let fetch_stream_id =
    // fetch_stream.fetch_stream_id.clone();

    //     let open_fetch_stream_response = timeout(Duration::from_millis(100),
    // service_stream.next())         .await
    //         .unwrap()
    //         .unwrap()
    //         .unwrap()
    //         .into_response()
    ...
    //     let fetch_response = timeout(Duration::from_millis(100), service_stream.next())
    //         .await
    //         .unwrap()
    //         .unwrap()
    //         .unwrap();
