...
use crate::ingest_v2::ingester::{Position, ShardStatus};
...
impl PrimaryShardTestExt for PrimaryShard {
    ...
    #[track_caller]
    fn assert_is_open(&self, expected_position: impl Into<Position>) {
        let expected_position = expected_position.into();
        let shard_status = self.shard_status_tx.borrow();
        assert_eq!(
            *shard_status,
            ShardStatus::Open(expected_position),
            "expected open primary shard at `{expected_position:?}`, got `{:?}`",
            *shard_status
        );
    }
}
...
impl ReplicaShardTestExt for ReplicaShard {
    ...
    fn assert_is_open(&self, expected_position: impl Into<Position>) {
        ...
        assert_eq!(
            *shard_status,
            ShardStatus::Open(expected_position),
            "expected open replica shard at `{expected_position:?}`, got `{:?}`",
            *shard_status
        );
    }
}
