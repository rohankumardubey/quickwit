...
fn build_query_filter(mut sql: String, query: &ListSplitsQuery) -> String {
    // ListSplitsQuery cannot be built on an empty index list.
    assert!(!query.index_uids.is_empty());
    sql.push_str(" WHERE (");
...
/// Builds an SQL query that returns indexes which match at least one pattern in
...
fn build_index_id_patterns_sql_query(index_id_patterns: Vec<String>) -> anyhow::Result<String> {
    ...
    let mut like_or_query: String = String::new();
    for (index_id_pattern_idx, index_id_pattern) in index_id_patterns.iter().enumerate() {
        ...
        let _ = write!(like_or_query, "index_id LIKE '{sql_pattern}'");
        if index_id_pattern_idx < index_id_patterns.len() - 1 {
            like_or_query.push_str(" OR ");
        }
    }
    Ok(format!("SELECT * FROM indexes WHERE {like_or_query}"))
}
...
mod tests {
    ...
    fn test_combination_sql_query_builder() {
        ...
        let query =
            ListSplitsQuery::for_indexes(vec![index_uid.clone(), index_uid_2.clone()]).unwrap();
        let sql = build_query_filter(String::new(), &query);
        assert_eq!(
            sql,
    ...
    fn test_index_id_pattern_like_query() {
        ...
        assert_eq!(
            ...
            .unwrap(),
            "SELECT * FROM indexes WHERE index_id LIKE '%-index-%-last%' OR index_id LIKE \
             'another-index'"
        );
        assert_eq!(
            &build_index_id_patterns_sql_query(vec![
