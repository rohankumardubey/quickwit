...
use quickwit_proto::{
    FetchDocsRequest, FetchDocsResponse, Hit, IndexUid, LeafHit, LeafListTermsRequest,
    LeafListTermsResponse, LeafSearchRequest, LeafSearchResponse, ListTermsRequest,
    ListTermsResponse, PartialHit, SearchRequest, SearchResponse, SnippetRequest, SortField,
...
pub async fn root_search(
    ...
    if indexes_metadatas.is_empty() {
        return Err(SearchError::IndexPatternsDoNotExist {
...
mod tests {
    ...
    use quickwit_config::{DocMapping, SearcherConfig};
    use quickwit_indexing::MockSplitBuilder;
    use quickwit_metastore::{IndexMetadata, MockMetastore};
    use quickwit_proto::{ScrollRequest, SortOrder, SortValue, SplitSearchError};
    use quickwit_query::query_ast::{qast_string_helper, query_ast_from_user_text};
    ...
    fn test_validate_request_and_build_metadatas_ok() {
        let search_request = quickwit_proto::SearchRequest {
            ...
            query_ast: qast_string_helper("body:test", &[]),
            max_hits: 10,
            start_offset: 10,
            ..Default::default()
        };
        let index_metadata_1 = IndexMetadata::for_test("test-index-1", "ram:///test-index-1");
        let mut index_metadata_with_different_default_fields =
            IndexMetadata::for_test("test-index-2", "ram:///test-index-2");
        index_metadata_with_different_default_fields
            .index_config
            .search_settings
            .default_search_fields = vec!["timestamp".to_string()];
    ...
    async fn test_root_search_multi_indices() -> anyhow::Result<()> {
        ...
        metastore.expect_list_indexes_metadatas().return_once(
            move |index_ids_query: ListIndexesQuery| {
                ...
                Ok(vec![index_metadata_1.clone(), index_metadata_2])
            },
        );
        metastore.expect_list_splits().return_once(move |filter| {
            assert_eq!(
                filter.index_uids,
                vec![index_uid_1.clone(), index_uid_2.clone()]
            );
            Ok(vec![
        ...
        mock_search_service_1
            ...
            .returning(|leaf_search_req: quickwit_proto::LeafSearchRequest| {
                assert!(
                    leaf_search_req.index_uri.contains("test-index-1")
                        || leaf_search_req.index_uri.contains("test-index-2")
                );
                let partial_hit_split_id = &leaf_search_req.split_offsets[0].split_id;
                Ok(quickwit_proto::LeafSearchResponse {
                    num_hits: 1,
                    partial_hits: vec![mock_partial_hit(partial_hit_split_id, 3, 1)],
                    failed_splits: Vec::new(),
        ...
        mock_search_service_1
            .expect_fetch_docs()
            .times(2)
            .returning(|fetch_docs_req: quickwit_proto::FetchDocsRequest| {
                assert!(
                    fetch_docs_req.index_uri.contains("test-index-1")
                        || fetch_docs_req.index_uri.contains("test-index-2")
                );
                Ok(quickwit_proto::FetchDocsResponse {
                    hits: get_doc_for_fetch_req(fetch_docs_req),
                })
