    ...
    quote! {
        ...
        pub mod mock {
            ...
            struct #mock_wrapper_name {
                inner: std::sync::Arc<#mock_name>
            }

            struct MockPtr(*mut #mock_name);

            unsafe impl Send for MockPtr {}

            impl std::ops::Deref for MockPtr {
                type Target = #mock_name;
    ...
    for syn_method in &context.methods {
        ...
        let method = quote! {
            async fn #method_name(&mut self, request: #request_type) -> #result_type<#response_type> {
                ...
                async move {
                    let mut mock_ptr = MockPtr(std::sync::Arc::into_raw(mock) as *mut #mock_name);
                    unsafe { std::sync::Arc::decrement_strong_count(mock_ptr.0); }
                    mock_ptr.#method_name(request).await
                }.await
            }
        };
        stream.extend(method);
